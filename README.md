...
- **社交行为决策（0.3.0 重构）**：本地概率不再是"关键词分数"，而是五个具名社交因素的乘积——
  `addressed_to_me`（是否在跟机器人说话，被引用回复直接拉满）、`conversation_relevance`
  （话题相关性，**关键词只做召回，缺席不再禁言**）、`conversation_activity`（刷屏压制）、
  `social_opportunity`（提问 / 安静时机加分）、`recent_reply_frequency`（5/30 分钟发言频率
  越高越克制）。因素值随决策 prompt 一并给模型参考。
- **两阶段生成（0.3.0）**：决策模型只输出"是否参与"的 JSON（不写回复）；确定 SPEAK 后由
  独立的回复生成调用产出正文——该调用自动附带 AstrBot 当前人格，说话风格完全由人格决定。
- **克制的主动参与**：本地规则先过滤，只有部分机会进入 LLM 决策；决策是否发声最终由模型把关。
- **决策追踪链（verbose_log）**：开启后每次决策输出一行中文 `[决策]` 日志——门拦截（10 种原因
  均翻译并带剩余秒数，**按原因去重不刷屏**）、概率掷骰（概率 vs 点数 + 六因素得分拆解）、
  模型判断（IGNORE/WAIT/SPEAK 及理由）、两阶段失败、延迟阶段取消、限流、发送退避、
  成功发言（今日额度与冷却）；关闭时零开销。
- **三态决策**：`IGNORE` / `WAIT` / `SPEAK`，强制 JSON 输出，带括号平衡容错解析；SPEAK 的正文由第二阶段生成。
...
- **活跃度反向调节**：群越刷屏，越不插话。
- **兴趣关键词**：只做"发现话题"的召回信号（提高相关性、辅助会话聚类），不再决定是否说话；支持负向词与每群覆盖。
...
| `verbose_log` | bool | `false` | 开启中文 `[决策]` 追踪日志；拦截按原因去重 |
